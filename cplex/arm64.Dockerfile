...
COPY cos_installer_linux-arm64.bin /tmp/installer
...
# For Apple M1 (arm64)
ENV PATH ${PATH}:${COSDIR}/cplex/bin/arm64_linux
ENV PATH ${PATH}:${COSDIR}/cpoptimizer/bin/arm64_linux
ENV PATH ${PATH}:${COSDIR}/opl/bin/arm64_linux
...
#ENV PYTHONPATH ${PYTHONPATH}:${COSDIR}/cplex/python/${CPX_PYVERSION}/arm64_linux
...
COPY model.mod ./model.mod
COPY input.xlsm ./input.xlsm
